...
class block:
    ...
    def __init__(self, index, parent_hash, target, data):

        self.index = index
        self.parent_hash = bytes.fromhex(parent_hash)
        self.target = target
...
parent_hash = input('Enter hash of parent block: ')
target = int(input('Enter target: '), 16)
data = input('Enter path to block body file: ')

max_nonce = 100000000000

block1 = block(index, parent_hash, target, data)
